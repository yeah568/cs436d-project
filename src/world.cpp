...
bool World::init(vec2 screen) {
    ...
    if (sounds_player.load_sounds()) {

        // Playing background music undefinitely
        // TODO: make different objects of background_music_player for different levels for right now, the background music is just coming from a general sound player
        sounds_player.play_level_one_music();
...
void World::destroy() {
    ...
    sounds_player.free_sounds();
    sounds_player.close_audio();
...
bool World::update(float elapsed_ms) {
    ...
    for (const auto &turtle : m_turtles) {
        if (m_salmon.collides_with(turtle)) {
            if (m_salmon.is_alive()) {
                sounds_player.play_player_death_sound();
            }
            m_salmon.kill();
            break;
        }
    ...
    while (fish_it != m_fish.end()) {
        if (m_salmon.collides_with(*fish_it)) {
            ...
            m_salmon.light_up();
            sounds_player.play_enemy_death_sound();
            ++m_points;
        } else
            ++fish_it;
    }
